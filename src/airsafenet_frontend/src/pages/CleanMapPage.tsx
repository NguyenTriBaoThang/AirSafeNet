...
function wardPolygons(w: WardSeed) { return w.geometry.type === "Polygon" ? [w.geometry.coordinates] : w.geometry.coordinates; }
...
function pathOf(w: WardSeed) { return wardPolygons(w).flatMap((polygon) => polygon.map((ring) => ring.map(([lon, lat], i) => { const p = project(lon, lat); return `${i ? "L" : "M"}${p.x.toFixed(1)} ${p.y.toFixed(1)}`; }).join(" ") + " Z")).join(" "); }
function districtPolygons(geometry: DistrictBoundaryGeometry): number[][][][] { return geometry.type === "Polygon" ? [geometry.coordinates as number[][][]] : geometry.coordinates as number[][][][]; }
function districtPathOf(geometry: DistrictBoundaryGeometry) { return districtPolygons(geometry).flatMap(poly => poly.map(ring => ring.map(([lon, lat], i) => { const p = project(lon, lat); return `${i ? "L" : "M"}${p.x.toFixed(1)} ${p.y.toFixed(1)}`; }).join(" ") + " Z")).join(" "); }
...
function WardMap({ wards, activeId, mode, zoom, route, onSelect }: { wards: WardAir[]; activeId: string | null; mode: Mode; zoom: number; route: RouteOption | null; onSelect: (id: string | null) => void }) {
  ...
  return <svg className="cm-map" viewBox={vb} role="img" aria-label="Bản đồ nền kiểu Google Map với lớp heatmap AQI/PM2.5 168 phường/xã/đặc khu TP.HCM" onClick={() => onSelect(null)}>
    ...
    <g className="cm-district-boundary-layer" aria-hidden="true">{HCMC_DISTRICT_BOUNDARIES.map(boundary => <path key={`district-boundary-${boundary.id}`} d={districtPathOf(boundary.geometry)} className="cm-district-boundary" />)}</g>
