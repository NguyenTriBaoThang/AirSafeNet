...
import type { WardSeed } from "../data/hcmcWardAirMap";
...
function coords(w: WardSeed) { return w.geometry.coordinates[0]; }
...
function pathOf(w: WardSeed) { return coords(w).map(([lon, lat], i) => { const p = project(lon, lat); return `${i ? "L" : "M"}${p.x.toFixed(1)} ${p.y.toFixed(1)}`; }).join(" ") + " Z"; }
...
function WardMap({ wards, activeId, mode, zoom, route, onSelect }: { wards: WardAir[]; activeId: string | null; mode: Mode; zoom: number; route: RouteOption | null; onSelect: (id: string | null) => void }) {
  ...
  return <svg className="cm-map" viewBox={vb} role="img" aria-label="Bản đồ nền kiểu Google Map với lớp heatmap AQI/PM2.5 113 phường TP.HCM" onClick={() => onSelect(null)}>
...
function Ranking({ wards, activeId, onSelect }: { wards: WardAir[]; activeId: string | null; onSelect: (id: string) => void }) { const best = [...wards].sort((a, b) => a.aqi - b.aqi).slice(0, 5), worst = [...wards].sort((a, b) => b.aqi - a.aqi).slice(0, 5); const col = (title: string, list: WardAir[]) => <div className="cm-ranking__col"><h3>{title}</h3>{list.map((w, i) => <button key={w.id} className={activeId === w.id ? "active" : ""} onClick={() => onSelect(w.id)}><span>{i + 1}</span><strong>{w.name}</strong><em style={{ color: color(w.risk) }}>AQI {w.aqi}</em></button>)}</div>; return <section className="cm-ranking">{col("Top 5 phường sạch nhất", best)}{col("Top 5 phường ô nhiễm cao", worst)}</section>; }
...
export default function CleanMapPage() {
  ...
  const [activeId, setActiveId] = useState<string | null>("q1_ben-thanh");
  ...
  return <main className="cm-page"><header className="cm-header"><div><span className="cm-eyebrow">Clean Route & Ward Air Quality Map</span><h2>Bản đồ AQI/PM2.5 theo {HCMC_WARD_COUNT} phường TP.HCM</h2><p>{computing ? "AI district cache đang được khởi tạo; bản đồ vẫn hiển thị bằng lớp ước tính để demo." : `Dữ liệu cập nhật ${lastUpdated} · ${HCMC_WARD_LAYER_SOURCE}`}</p></div><div className="cm-header__stats"><div><span>AQI trung bình</span><strong style={{ color: color(avgAqi) }}>{avgAqi}</strong></div><div><span>Sạch nhất</span><strong>{best?.name} · AQI {best?.aqi}</strong></div><div><span>Ô nhiễm cao</span><strong>{worst?.name} · AQI {worst?.aqi}</strong></div></div></header><div className="cm-toolbar"><div className="cm-mode-tabs" role="tablist" aria-label="Cấp zoom bản đồ">{[{ id: "city", text: "Cấp thành phố" }, { id: "ward", text: "Cấp phường" }, { id: "route", text: "Cấp tuyến đường" }].map(x => <button key={x.id} className={mode === x.id ? "active" : ""} onClick={() => setMode(x.id as Mode)} type="button">{x.text}</button>)}</div><div className="cm-zoom-controls"><button type="button" onClick={() => setZoom(v => clamp(+(v - 0.25).toFixed(2), 1, 2.8))}>-</button><span>Zoom {zoom.toFixed(2)}x</span><button type="button" onClick={() => setZoom(v => clamp(+(v + 0.25).toFixed(2), 1, 2.8))}>+</button></div></div><section className="cm-layout"><div className="cm-map-panel"><WardMap wards={wards} activeId={activeId} mode={mode} zoom={zoom} route={selectedRoute} onSelect={selectWard} /><Legend />{loading && <div className="cm-map-loading">Đang đồng bộ dữ liệu AQI...</div>}</div><aside className="cm-side-panel">{activeWard ? <WardDetail ward={activeWard} /> : <section className="cm-empty-detail"><h3>Chọn một phường trên bản đồ</h3><p>Bấm vào vùng màu để xem AQI, PM2.5, nguồn dữ liệu, confidence, khuyến nghị sức khỏe và giờ sạch hơn trong ngày.</p></section>}<Ranking wards={wards} activeId={activeId} onSelect={selectWard} /></aside></section><RoutePlanner wards={wards} startInput={startInput} endInput={endInput} departureTime={departureTime} vehicleId={vehicleId} profileId={profileId} selectedRouteId={selectedRouteId} routes={routes} startWard={startWard} endWard={endWard} onStart={setStartInput} onEnd={setEndInput} onTime={setDepartureTime} onVehicle={setVehicleId} onProfile={setProfileId} onRoute={selectRoute} /></main>;
}
